...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        if (getNode(leftIndex(index)) == null && getNode(rightIndex(index)) == null) {return;}

        int target = min(leftIndex(index), rightIndex(index));
        Node parent = getNode(index);
        Node child = getNode(target);
    ...
    public void changePriority(T item, double priority) {
        ...
        for (int i = 1; i <= size; ++i) {
            Node compare = getNode(i);
            if (compare.item().equals(item)) {
                double prev = compare.priority();
                compare.myPriority = priority;
                if (priority < prev) {
                    swim(i);
                } else {
                    sink(i);
